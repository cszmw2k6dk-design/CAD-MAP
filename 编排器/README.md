...
（同一张支架的几行**连号**，例：一张支架拆 3 行就是 STR01/02/03，下一张接着 STR04…）。
不拆 = 一个框一个号（老行为）。

- 哪一列属于哪一类按**框长**认：支架框长边 ÷ 页高 分档，档之间的比例对上各类的「长度FT」比例，
  **长的那一档就是长度 FT 大的那类**（本项目 13 串 214.3FT 比 9 串 100.3FT 长一倍多）；
- 整册只分出一档、且只有一类要拆时，就按要拆的那类拆（程序日志里会写明）；
