...
**按汇流箱(Box)象限定编号方向**（「支架」页下面四个「象限 … 的顺序」下拉，默认开）：
图纸上每页的汇流箱（识别结果 `yolo_box_detection_results` 里的 `Box` 小框）周围常围着好几个
LBD 区域，散在不同方向，整片区域都用同一个方向编号时往往有一半是反的。这一版让每个 LBD 区域
按自己落在汇流箱的第几象限单独选编号方向：

- **总开关**：「按汇流箱(Box)四象限规律命名」（默认开）。关掉 = **这一段全部不生效**、
  一律按上面的「STR 编号顺序」编号（四个下拉置灰但选值留着，重新打开就是原来的）；
- **原点** = 本页汇流箱(Box)的**几何中心**（一页有多个汇流箱时，取离这个区域最近的那个）；
- **判据** = 这个 **LBD 区域框的中心**；轴用图纸的正交轴（u 右为正、v 上为正）；
- **象限** = I 右上 / II 左上 / III 左下 / IV 右下，四个象限各一个下拉，选 8 种顺序里的哪一种；
  某个象限选「0 用全局顺序」就是**那个象限不改**（和以前一样）；
- **死区**：区域中心离原点不到「汇流箱短边 × 0.5」算压在轴上、判不准 → 用全局顺序；
- **兜底**：这一页没有汇流箱（老版本 JSON 里没有 `Box` 这一段）、或者四个都选 0 →
  完全按上面的「STR 编号顺序」走，输出和以前逐字节一样（可以直接对比验证）；
- 默认给的是**近端起步**：近端 = 区域里离汇流箱最近的那个角（引线进来的一头），从它开始
  往远端走 —— 右上 = 4、左上 = 8、左下 = 7、右下 = 2；方向反了就把那个象限的下拉换掉就行。

**象限预览**（「标签」页「刷新象限预览」）：画的是**规律示意图**，不依赖识别结果、也不用先选 JSON ——
中间是原点（汇流箱 Box 的几何中心），四个象限各画一小片 3 列 x 3 行的格子，格子里的 `01~09`
就是这个象限当前选的顺序的走法（连线就是编号走向），抬头写这一象限用哪条顺序。
换一个下拉，那个象限的号立刻换位置，8 种顺序的差别一眼能看出来；总开关关掉时四个象限都按
全局顺序画，抬头也会写明「总开关 = 关」。
...
2. 生成布局、插件对准完之后，CAD 侧再用这个范围**重新对准一次视口**（居中 + 按区域上下限缩放），
   布局视口里看到的就基本是 LBD 区域本身。留白按「PDF」页的
